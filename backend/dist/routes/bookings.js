...
const checkAvailability = async (vehicleId, startAt, endAt, excludeBookingId) => {
...
const calculateCost = (startAt, endAt, dailyRate, type) => {
...
router.put("/:id/mark-hired", auth_1.authenticate, auth_1.requireAdmin, async (req, res, next) => {
    try {
        const booking = await prisma.$transaction(async (tx) => {
...
router.put("/:id/mark-returned", auth_1.authenticate, auth_1.requireAdmin, async (req, res, next) => {
    try {
        const booking = await prisma.$transaction(async (tx) => {
            const updated = await tx.booking.update({
                where: { id: Number.parseInt(req.params.id) },
...
// Cancel booking
router.put("/:id/cancel", auth_1.authenticate, async (req, res, next) => {
    try {
